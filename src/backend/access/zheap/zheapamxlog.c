...
zheap_xlog_insert(XLogReaderState *record)
{
	...
	if (!skip_undo)
	{
		...
		urecptr = PrepareUndoInsert(&undorecord, UNDO_PERMANENT, xid, record);
		InsertPreparedUndo();
...
zheap_xlog_update(XLogReaderState *record)
{
	...
	else
	{
		...
		urecptr = PrepareUndoInsert(&undorecord, UNDO_PERMANENT, xid, record);
		newurecptr = PrepareUndoInsert(&newundorecord, UNDO_PERMANENT, xid, record);
...
zheap_xlog_multi_insert(XLogReaderState *record)
{
	...
	if (!skip_undo)
	{
		...
		{
			undorecord[i].uur_blkprev = urecptr;
			urecptr = PrepareUndoInsert(&undorecord[i], UNDO_PERMANENT, xid, record);
		}
